...
struct battery_chg_dev {
	...
	/* bypass charging: 0=normal, 1=input_suspended, 2=bypass */
	int				bypass_charging;
};
					...
					int val)
{
	...
	/* When bypass charging is active, override thermal level to 0 */
	if (bcdev->bypass_charging == 2) {
		...
		struct class_attribute *attr, const char *buf, size_t count)
{
	...
	switch (val) {
	case 0:
		/* Normal charging: clear input suspend and re-enable charging */
		rc = write_property_id(bcdev, &bcdev->psy_list[PSY_TYPE_XM],
					XM_PROP_INPUT_SUSPEND, 0);
		if (rc < 0)
			return rc;

		/* Re-enable charging IC if we were in bypass mode */
		if (bcdev->bypass_charging == 2) {
			rc = write_property_id(bcdev,
					&bcdev->psy_list[PSY_TYPE_BATTERY],
					BATT_CHG_CTRL_EN, 1);
			if (rc < 0)
				pr_err("Failed to re-enable charging, rc=%d\n", rc);
		}
		bcdev->bypass_charging = 0;
		break;

	case 1:
		/* Input suspend: stop charging and USB input (original behavior) */
		rc = write_property_id(bcdev, &bcdev->psy_list[PSY_TYPE_XM],
					XM_PROP_INPUT_SUSPEND, 1);
		if (rc < 0)
			return rc;
		bcdev->bypass_charging = 1;
		break;

	case 2:
		/* Bypass charging: keep USB input active, disable charging IC */
		/* Ensure USB input is NOT suspended */
		rc = write_property_id(bcdev, &bcdev->psy_list[PSY_TYPE_XM],
					XM_PROP_INPUT_SUSPEND, 0);
		if (rc < 0)
			return rc;

		/* Disable charging IC - battery will not charge */
		rc = write_property_id(bcdev,
				&bcdev->psy_list[PSY_TYPE_BATTERY],
				BATT_CHG_CTRL_EN, 0);
		if (rc < 0) {
			pr_err("Failed to disable charging for bypass, rc=%d\n", rc);
			return rc;
		}

		/* Override thermal level to prevent interference */
		bcdev->curr_thermal_level = 0;
		bcdev->bypass_charging = 2;
		break;
	}

	pr_info("input_suspend set to %d (bypass_charging=%d)\n",
		val, bcdev->bypass_charging);
